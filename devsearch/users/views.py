...
def registerUser(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Ann error has occured during registration.')
...
def profiles(request):
    ...
    profiles_per_page = 6
    custom_range, profiles = paginateProfiles(request, profiles, profiles_per_page)
...
@login_required(login_url='login')
def inbox(request):
    '''Go to inbox section of a platform'''
    profile = request.user.profile
